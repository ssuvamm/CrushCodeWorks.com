...
## Validation in restricted CI/container environments
If dependency installation is blocked by registry/network policy, treat downstream build/test failures as **environment warnings** rather than code regressions.

Example reporting:
- ⚠️ `npm install` (registry/network restricted)
- ⚠️ `npm run build` (cannot run because dependencies are unavailable)
- ⚠️ Playwright screenshot (runtime/browser limitation in container)

When running locally or in CI with registry access, use:
```bash
npm ci
npm run check
npm run build
